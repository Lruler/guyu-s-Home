服务端渲染（Server-Side Rendering，简称SSR）是一种在服务器端生成HTML内容并将其发送给客户端的技术，客户端接收到预先渲染好的HTML后，直接展示页面内容。这与客户端渲染（Client-Side Rendering，简称CSR）相对应，客户端渲染是在浏览器端使用JavaScript生成和更新HTML内容。以下是服务端渲染的原理和过程：
